...
function [q, q_M_norm] = mode_acc_meth(time_prop, elements_All, nodes_All,...
...
t_final = 40;           % Final time of chirp   (int)[s]
...
% Extracting the truncation used before for the natural frequencies,
% and that will still be used here
tr = length(w_tr);
...
for t = 0:h:t_max
    ...
    % Extracting external force vector at current time
    p(shakerDOF_nbr,1) = p(shakerDOF_nbr,fix(t/h + 1));
    ...
    for mode = 1:tr                 % Sum for each truncated mode
        ...
        % Inhomogeneous part
        eta_p = trapz(prod_conv(mode,fix(t/h)+1));
...
elseif strcmp(pl,'y') == 1 || strcmp(pl,'Y') == 1
    ...
    title('Y-dimension') ; ylabel('Displacement [m]');
    
    subplot(2,3,3);
    plot(t,q(accDOFs_nbr(3),fix(t/h)+1));
    title('Z-dimension'); xlabel('Time [s]');
